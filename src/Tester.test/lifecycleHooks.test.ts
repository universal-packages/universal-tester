...
async function testBasicHookExecutionOrder() {
  ...
  const expectedOrder = ['before', 'beforeEach', 'test 1', 'afterEach', 'beforeEach', 'test 2', 'afterEach', 'after']
...
async function testNestedDescribeHooks() {
  ...
  const expectedOrder = ['outer before', 'inner before', 'outer beforeEach', 'inner beforeEach', 'nested test', 'outer afterEach', 'inner afterEach', 'inner after', 'outer after']
...
async function testHooksWithSkippedTests() {
  ...
  const expectedOrder = ['before', 'beforeEach', 'normal test', 'afterEach', 'after']
...
async function testHooksWithOnlyTests() {
  ...
  const expectedOrder = ['before', 'beforeEach', 'only test', 'afterEach', 'after']
...
async function testHooksWithRandomOrder() {
  ...
  if (beforeCount !== 1 || beforeEachCount !== 5 || afterEachCount !== 5 || afterCount !== 1 || testCount !== 5) {
    throw new Error(
      `Random hooks test failed - incorrect hook counts: before=${beforeCount}, beforeEach=${beforeEachCount}, afterEach=${afterEachCount}, after=${afterCount}, tests=${testCount}`
    )
  }
